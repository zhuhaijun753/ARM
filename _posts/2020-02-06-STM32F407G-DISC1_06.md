...
HSI는 내부 16Mhz RC 오실레이터로 동작한다. SYSCLK는 물론, PLL의 입력으로도 쓰일 수 있다. __값싸고 빠른 Startup 시간이 HSE보다 장점이지만__ (`HSI는 전원이 들어올 때부터 동작되는 main system clock`), calibration을 통한 클럭도 정확하지 않다는 단점이 있다.__
